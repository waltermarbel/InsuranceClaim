...
const ClauseTag: React.FC<{ 
    ...
    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold border shadow-sm transition-all group focus-within:opacity-100 ${colorClass} ${borderClass} ${textClass}`}>
        {text}
        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="ml-1.5 opacity-0 group-hover:opacity-100 focus:opacity-100 hover:text-red-600 transition-opacity focus-visible:ring-2 focus-visible:ring-red-500 outline-none rounded-full" aria-label="Remove item" title="Remove item">
                    ...
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                        {(policy.coverage || []).map((limit, index) => (
                             <div key={index} className="flex items-center gap-2 text-xs bg-white p-2 rounded border border-slate-200 hover:border-primary/40 transition-colors group">
                                ...
                                <button onClick={() => removeCoverage(index)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity p-1 focus-visible:ring-2 focus-visible:ring-red-500 outline-none rounded-md" aria-label="Remove coverage" title="Remove coverage">
                        ...
                        <ul className="space-y-2">
                            {filterClauses(policy.conditions).map((condition, i) => (
                                <li key={i} className="text-xs text-slate-700 bg-amber-50 p-2 rounded border border-amber-100 flex items-start gap-2 group focus-within:opacity-100">
                                    ...
                                    <button onClick={() => removeClause('conditions', (policy.conditions || []).indexOf(condition))} className="opacity-0 group-hover:opacity-100 focus:opacity-100 text-amber-400 hover:text-amber-600 focus-visible:ring-2 focus-visible:ring-amber-500 outline-none rounded-full" aria-label="Remove clause" title="Remove clause"><XCircleIcon className="h-3 w-3"/></button>
                                </li>
                            ))}
