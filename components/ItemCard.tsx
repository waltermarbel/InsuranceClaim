...
const ItemCard: React.FC<ItemCardProps> = ({ item, onSelect, onApprove, onReject, isSelected, onToggleSelection }) => {
  ...
  return (
    <div
      className={`relative group bg-white rounded-lg shadow-sm border overflow-hidden transform hover:-translate-y-1 transition-all duration-300 ${isSelected ? 'border-primary ring-2 ring-primary' : 'border-slate-200'}`}
    >
        <div 
          className="absolute top-2 left-2 z-10 bg-white/50 backdrop-blur-sm rounded-md p-1"
        ...
        >
            <input 
                ...
                className="h-5 w-5 rounded border-slate-400 text-primary focus:ring-primary shadow"
        ...
        <div onClick={onSelect} className="cursor-pointer">
          ...
          <div className="p-3 border-t border-slate-100">
             <div className="flex items-center gap-2">
                <ItemIcon className="h-4 w-4 text-medium flex-shrink-0" title={`Category: ${item.itemCategory}`} />
                <p className="text-sm font-medium text-dark truncate" title={item.itemName}>
                    {item.itemName}
                </p>
            </div>
            <p className="text-xs text-medium mt-1 truncate pl-6" title={item.itemDescription}>
