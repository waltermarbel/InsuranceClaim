                                    ...
                                    {selectedEvent.linkedDocumentIds.map(docId => {
                                        ...
                                        return d ? (
                                            <div key={docId} className="flex justify-between items-center text-xs bg-white border border-slate-200 p-2 rounded">
                                                ...
                                                <button aria-label="Remove linked document" onClick={() => handleUpdateEvent(selectedEvent.id, { linkedDocumentIds: selectedEvent.linkedDocumentIds.filter(id => id !== docId) })} className="text-slate-400 hover:text-rose-500 p-1 focus-visible:ring-2 focus-visible:outline-none rounded">&times;</button>
                                            </div>
                                        ) : null;
                                    })}
                                </div>
                                    ...
                                    {selectedEvent.linkedItemIds?.map(itemId => {
                                        ...
                                        return item ? (
                                            <div key={itemId} className="flex justify-between items-center text-xs bg-white border border-slate-200 p-2 rounded">
                                                ...
                                                <button aria-label="Remove linked item" onClick={() => handleUpdateEvent(selectedEvent.id, { linkedItemIds: (selectedEvent.linkedItemIds || []).filter(id => id !== itemId) })} className="text-slate-400 hover:text-rose-500 p-1 focus-visible:ring-2 focus-visible:outline-none rounded">&times;</button>
                                            </div>
                                        ) : null;
                                    })}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {selectedEvent.involvedPersons.map((person, idx) => (
                                        <span key={idx} className="flex items-center gap-1 text-xs bg-white border border-slate-200 px-2 py-1 rounded-full text-slate-700 font-medium">
                                            ...
                                            }} className="text-slate-400 hover:text-rose-500 font-bold ml-1 p-1 focus-visible:ring-2 focus-visible:outline-none rounded">&times;</button>
                                        </span>
                                    ))}
                                </div>
                                <input 
