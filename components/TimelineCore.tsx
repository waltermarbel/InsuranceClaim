                ...
                {selectedEvent ? (
                    <div className="bg-white p-6 justify-between rounded-xl shadow-sm border border-slate-200">
                        <div className="flex justify-between items-start mb-4 pb-4 border-b border-slate-100">
                            ...
                            <button onClick={() => handleRemoveEvent(selectedEvent.id)} aria-label="Remove event" title="Remove event" className="text-rose-500 hover:bg-rose-50 p-1.5 rounded transition"><TrashIcon className="h-4 w-4"/></button>
                        </div>
                                    ...
                                    {selectedEvent.linkedItemIds?.map(itemId => {
                                        ...
                                        return item ? (
                                            <div key={itemId} className="flex justify-between items-center text-xs bg-white border border-slate-200 p-2 rounded">
                                                ...
                                                <button onClick={() => handleUpdateEvent(selectedEvent.id, { linkedItemIds: (selectedEvent.linkedItemIds || []).filter(id => id !== itemId) })} aria-label="Remove linked item" title="Remove linked item" className="text-slate-400 hover:text-rose-500">&times;</button>
                                            </div>
                                        ) : null;
                                    })}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {selectedEvent.involvedPersons.map((person, idx) => (
                                        <span key={idx} className="flex items-center gap-1 text-xs bg-white border border-slate-200 px-2 py-1 rounded-full text-slate-700 font-medium">
                                            ...
                                            }} aria-label={`Remove person ${person}`} title={`Remove person ${person}`} className="text-slate-400 hover:text-rose-500 font-bold ml-1">&times;</button>
                                        </span>
                                    ))}
                                </div>
                                <input 
