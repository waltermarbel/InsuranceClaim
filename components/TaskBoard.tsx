    ...
    return (
        <div className={`bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col ${compact ? '' : 'h-full'}`}>
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <div className="flex bg-white rounded-lg p-0.5 border border-slate-200" role="group" aria-label="Filter tasks">
                    <button 
                        onClick={() => setView('pending')} 
                        aria-pressed={view === 'pending'}
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${view === 'pending' ? 'bg-slate-100 text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Pending
                    </button>
                    <button 
                        ...
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${view === 'completed' ? 'bg-slate-100 text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Done
                    </button>
                </div>
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-2 min-h-[150px]">
                {filteredTasks.length === 0 ? (
                    ...
                    filteredTasks.map(task => {
                        ...
                        return (
                            <div key={task.id} className="group flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:border-primary/30 hover:bg-slate-50/50 focus-within:border-primary/30 focus-within:bg-slate-50/50 transition-all">
                                <button 
                                    ...
                                    className={`flex-shrink-0 mt-0.5 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${task.isCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300 hover:border-primary'}`}
                                >
                                ...
                                <button 
                                    ...
                                    title="Delete task"
                                    className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500/50 transition-opacity rounded-sm"
                                >
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                        ...
                        <button 
                            ...
                            aria-label="Add task"
                            title="Add task"
                            className="bg-primary text-white p-2 rounded-lg hover:bg-primary-dark transition disabled:opacity-50 shadow-sm flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:ring-offset-1"
                        >
