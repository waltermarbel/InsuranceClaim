            ...
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <div className="flex bg-white rounded-lg p-0.5 border border-slate-200">
                    <button 
                        ...
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-primary/50 ${view === 'pending' ? 'bg-slate-100 text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Pending
                    </button>
                    <button 
                        ...
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-primary/50 ${view === 'completed' ? 'bg-slate-100 text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Done
                    </button>
                </div>
            ...
            <div className="flex-grow overflow-y-auto p-4 space-y-2 min-h-[150px]">
                {filteredTasks.length === 0 ? (
                    ...
                    filteredTasks.map(task => {
                        ...
                        return (
                            <div key={task.id} className="group focus-within:bg-slate-50/50 flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:border-primary/30 hover:bg-slate-50/50 transition-all">
                                <button 
                                    onClick={() => handleToggle(task.id)}
                                    aria-label={task.isCompleted ? "Mark task as incomplete" : "Mark task as complete"}
                                    title={task.isCompleted ? "Mark as incomplete" : "Mark as complete"}
                                    className={`flex-shrink-0 mt-0.5 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none focus-visible:ring-primary/50 ${task.isCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-slate-300 hover:border-primary'}`}
                                >
                                ...
                                <button 
                                    ...
                                    title="Delete task"
                                    className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:ring-2 focus-visible:outline-none focus-visible:ring-rose-500/50 transition-opacity rounded p-1"
                                >
