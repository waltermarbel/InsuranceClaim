            ...
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <div className="flex bg-white rounded-lg p-0.5 border border-slate-200">
                    <button 
                        ...
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${view === 'pending' ? 'bg-slate-100 text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Pending
                    </button>
                    <button 
                        onClick={() => setView('completed')} 
                        className={`px-3 py-1 text-xs font-semibold rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary ${view === 'completed' ? 'bg-slate-100 text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Done
                    </button>
                </div>
                        ...
                        return (
                            <div key={task.id} className="group flex items-start gap-3 p-3 rounded-lg border border-slate-100 hover:border-primary/30 hover:bg-slate-50/50 focus-within:border-primary/30 focus-within:bg-slate-50/50 transition-all">
                                ...
                                <button 
                                    ...
                                    className="text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:text-rose-500 rounded transition-all"
                                >
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                        <div className="flex-grow relative">
                            ...
                            <div className="absolute right-1 top-1">
                                <select 
                                    ...
                                    className="text-[10px] font-bold bg-slate-100 border-none rounded py-1 pl-2 pr-1 cursor-pointer focus:ring-2 focus:ring-primary/50 text-slate-600 h-7"
                                >
                        ...
                        <button 
                            ...
                            title="Add task"
                            aria-label="Add task"
                            className="bg-primary text-white p-2 rounded-lg hover:bg-primary-dark transition disabled:opacity-50 shadow-sm flex-shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
                        >
                    ...
                    {!filterItemId && (
                        <div className="flex items-center gap-2 px-1">
                            ...
                            <select
                                ...
                                title="Link task to specific item"
                                aria-label="Link task to specific item"
                                className="text-xs bg-transparent border-none text-slate-500 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 rounded cursor-pointer p-0 w-full truncate"
                            >
