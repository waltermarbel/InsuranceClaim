...
const RichTextEditor: React.FC<{ value: string; onBlur: (val: string) => void }> = ({ value, onBlur }) => {
    ...
    useEffect(() => {
        if (editorRef.current && editorRef.current.innerHTML !== value) {
            // SECURITY: Sanitize rich text input before assigning to innerHTML to prevent XSS attacks
    ...
    const handleBlur = () => {
        if (editorRef.current) {
            // SECURITY: Sanitize HTML content retrieved from DOM before saving state to prevent stored XSS
...
const ClaimDetailsEditor: React.FC<ClaimDetailsEditorProps> = ({ details, onUpdate, onGenerateNarrative, isGeneratingNarrative }) => {
    ...
    return (
        <div className="bg-white rounded-xl border border-primary/20 shadow-md p-6 mb-8 ring-1 ring-primary/5">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-slate-800 font-heading">Claim Details</h2>
            ...
            <div className="mt-8 border-t border-slate-200 pt-6">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <label className="block text-sm font-bold uppercase text-slate-800 tracking-wider">Claim Narrative</label>
                        <p className="text-xs text-slate-500 mt-1">A structured explanation of the incident aligning with policy triggers.</p>
                    </div>
                    {onGenerateNarrative && (
                        <button 
                            type="button"
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-indigo-50 border border-indigo-200 text-indigo-700 rounded-lg hover:bg-indigo-100 transition shadow-sm font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                ...
                <RichTextEditor 
                    ...
                    onBlur={(val) => onUpdate({ narrative: val })}
                />
            </div>
        </div>
    );
