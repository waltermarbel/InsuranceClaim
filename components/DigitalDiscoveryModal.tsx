...
import { connectGooglePhotos, connectGmail } from "../firebase.ts";
...
const DigitalDiscoveryModal: React.FC<DigitalDiscoveryModalProps> = ({
  onClose,
  onImport,
}) => {
  const dispatch = useAppDispatch();
  const [step, setStep] = useState<"connect" | "scanning" | "review">(
    "connect",
  );
  const [selectedSource, setSelectedSource] = useState<"email" | "photos">(
    "email",
  );
  ...
  const startDiscovery = async () => {
    setScanLog([
      `Initiating secure connection to ${selectedSource === "email" ? "Email Provider" : "Cloud Photos"}...`,
    ]);

    try {
      let accessToken;
      if (selectedSource === "email") {
        accessToken = await connectGmail();
      } else {
        accessToken = await connectGooglePhotos();
      }
      ...
      const items = await geminiService.performDigitalDiscovery(
        selectedSource,
        accessToken,
        (logLine) => {
          setScanLog((prev) => [...prev, logLine]);
        },
      );
      ...
      console.error(error);
      setStep("scanning"); // keep in scanning to show error
  ...
  const handleImport = () => {
    ...
    dispatch({
      type: "LOG_ACTIVITY",
      payload: {
        action: "DIGITAL_DISCOVERY",
        details: `Discovered ${finalItems.length} items from ${selectedSource}`,
        app: "Gemini",
      },
    });
    onClose();
            ...
            <div className="space-y-8">
              <div className="text-center space-y-2">
                <h3 className="text-lg font-bold text-slate-800">
                  Connect Your Digital Archives
                </h3>
                <p className="text-sm text-slate-500">
                  Allow AI to scan your history for forgotten assets, receipts,
                  and visual evidence.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-primary font-semibold mt-2">
                      Finds: Receipts, Order Confirmations
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-primary font-semibold mt-2">
                      Finds: Visual Evidence of Ownership
                    </p>
                  </div>
                </button>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 text-xs text-blue-800 flex items-start gap-2">
                ...
                <p>
                  <strong>Privacy First:</strong> The AI scans metadata locally
                  where possible. Only extracted asset details are stored in
                  your vault. No raw emails or photos are permanently saved
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-slate-800">
                  Found {discoveredItems.length} Potential Assets
                </h3>
              ...
              <div className="space-y-3">
                {discoveredItems.map((item) => (
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded flex items-center justify-center border mt-1 transition-colors ${selectedIds.has(item.id) ? "bg-primary border-primary" : "bg-white border-slate-300"}`}
                    >
                    ...
                    <div className="flex-grow">
                      <div className="flex justify-between items-start">
                        ...
                        <span className="font-bold text-emerald-600">
                          $
                          {(item.replacementCostValueRCV || 0).toLocaleString()}
                        </span>
                      </div>
                      <p className="text-xs text-slate-500 mt-1">
        ...
        <div className="p-6 border-t bg-slate-50 flex justify-end gap-3">
          {step === "connect" && (
            <>
              <button
                onClick={onClose}
                className="px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-white rounded-lg transition"
              >
                Cancel
              </button>
              <button
                onClick={startDiscovery}
                className="px-6 py-2 text-sm font-bold bg-primary text-white rounded-lg shadow-md hover:bg-primary-dark transition"
              >
                Start Scan
              </button>
            </>
          ...
          {step === "scanning" && (
            <button
              disabled
              className="px-6 py-2 text-sm font-bold bg-slate-300 text-white rounded-lg cursor-not-allowed"
            >
              Scanning...
            </button>
          )}
          {step === "review" && (
            <>
              ...
              <button
                ...
                className="px-6 py-2 text-sm font-bold bg-primary text-white rounded-lg shadow-md hover:bg-primary-dark transition disabled:opacity-50"
              >
