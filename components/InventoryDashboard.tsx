...
const InventoryDashboard: React.FC<InventoryDashboardProps> = ({ 
    ...
    const tableData = useMemo(() => {
        // Bolt Optimization: Evaluate searchTerm to lowercase once outside the filter loop
        const lowerSearchTerm = searchTerm?.toLowerCase() || '';

        // Bolt Optimization: Replace chained O(n) .filter() calls with a single-pass O(n) filter
        // using short-circuit evaluation to prevent excessive array memory allocation per render
        let data = inventory.filter(item => {
            // Text search
            if (lowerSearchTerm) {
                const nameMatch = item.itemName && item.itemName.toLowerCase().includes(lowerSearchTerm);
                const categoryMatch = item.itemCategory && item.itemCategory.toLowerCase().includes(lowerSearchTerm);
                if (!nameMatch && !categoryMatch) return false;
            }

            // Categories
            if (selectedCategories.size > 0 && (!item.itemCategory || !selectedCategories.has(item.itemCategory))) return false;
