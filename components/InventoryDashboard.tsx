...
const InventoryDashboard: React.FC<InventoryDashboardProps> = ({ 
    ...
    const tableData = useMemo(() => {
        // Optimize: Use single pass filtering instead of chained .filter() to prevent intermediate array allocations
        let data = inventory.filter(item => {
            if (searchTerm) {
                const searchLower = searchTerm.toLowerCase();
                const nameMatch = item.itemName?.toLowerCase().includes(searchLower);
                const catMatch = item.itemCategory?.toLowerCase().includes(searchLower);
