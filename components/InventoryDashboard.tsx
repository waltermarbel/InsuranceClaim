...
const InventoryDashboard: React.FC<InventoryDashboardProps> = ({ 
    ...
    // ⚡ Bolt Optimization: Use a single-pass filter with early returns to avoid O(k) array allocation overhead from chaining .filter()
    const tableData = useMemo(() => {
        let data = inventory.filter(item => {
            // Search filter
            const matchesSearch = (item.itemName && item.itemName.toLowerCase().includes(searchTerm?.toLowerCase() || '')) ||
