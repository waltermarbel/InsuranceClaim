...
const InventoryDashboard: React.FC<InventoryDashboardProps> = ({ 
    ...
    const tableData = useMemo(() => {
        // Optimize: single-pass filtering to avoid O(n) array allocations
        const searchStr = searchTerm?.toLowerCase() || '';

        let data = inventory.filter(item => {
            const matchesSearch =
                (item.itemName && item.itemName.toLowerCase().includes(searchStr)) ||
                (item.itemCategory && item.itemCategory.toLowerCase().includes(searchStr));
